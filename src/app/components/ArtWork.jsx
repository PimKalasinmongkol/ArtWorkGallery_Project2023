...
export default function ArtWork({ user }) {
  ...
  const getAllUser = async () => {
    try {
      const response = await fetch(
        "http://localhost:4000/user/getUserIdAndUserArtistname"
      );
  ...
  const getArtwork = async () => {
    try {
      const response = await fetch(
        "http://localhost:4000/gallery/getAllUserAndGallery"
      );
      const data = await response.json();
      setArtwork(data);
    } catch (error) {
  ...
  const getAllComment = async () => {
    try {
      const response = await fetch(
        "http://localhost:4000/gallery/getAllComment"
      );
      const data = await response.json();
      if (data.length > 0) {
        setAllComment(data);
  ...
  const handleLike = async (user_id, gallery_id) => {
    const isUserLikeThisGallery = allLike.find(
      (item) => item.gallery_id === gallery_id && item.user_id === user_id
    );
    ...
    if (isUserLikeThisGallery === undefined) {
      ...
      try {
        const response = await fetch(
          "http://localhost:4000/gallery/undo_like",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleCommentSend = async (user_id, gallery_id) => {
    if (commentContent.length == 0) {
      ...
      try {
        const response = await fetch(
          "http://localhost:4000/gallery/add_comment",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
                ...
                <div className="artWork-box" key={item.gallery_id}>
                  ...
                  <div className="artwork-image">
                    ...
                    <div className="artwork-info">
                      <p className="artwork-info-title">
                    ...
                    <div
                      className="artwork-like-comment"
                      href={`/infoWork/${item.gallery_id}`}
                    >
                      <div>
                        <FontAwesomeIcon
                          className="likeIcon"
